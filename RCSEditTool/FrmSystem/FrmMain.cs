...
namespace RCSEditTool
{
    public partial class FrmMain : BaseForm, ICanvasOwner, IEditToolOwner
    {
        ...
        #region 窗体自定义函数
        //
        private void reinitLay(float width, float height, float space,float len,bool storagefull,int girdnum)
        {
            m_data.reinitLay(width, height, space, len, storagefull, girdnum);
        }
        ...
        private void btnMapOption_ItemClick(object sender, ItemClickEventArgs e)
        {
            try
            {
                    using (FrmMapOption frm = new FrmMapOption())
                    {
                        frm.initData(width, height, space, girdnum);
                        if (frm.ShowDialog() == DialogResult.OK)
                        {
                        width = frm.width;
                        height = frm.height;
                        ...
                        len = frm.len;
                        girdnum = frm.girdnum;
                        bool storagefull = frm.storageFull;
                        if (frm.removeall)
                        {
                            removeAll();
                        }
                        reinitLay(frm.width,frm.height, frm.space,frm.len, storagefull, girdnum);
                            m_canvas.DoInvalidate(true);
                        }
                    }
