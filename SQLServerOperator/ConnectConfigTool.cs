...
{
    public class ConnectConfigTool
    {
        #region 属性
        public static DataBaseInfo DBase { get; set; }//初始化数据库信息
        public static ClientConfig serverconfig { get; set; }//通信配置
        //public static IDbOperator dbOperator { get; set; }//数据库操作变量
        #endregion
        ...
        public static DataTable TxtToDT()
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("数据库连接文件不存在!");
                }
        ...
        public static void setDBase()
        {
            try
            {
                ...
                if (dt != null && TxtToDT().Rows.Count > 0)
                {
                    ...
                    DBase = CreateDeepCopy<DataBaseInfo>(db);
                    IDbOperator dbOperator = CreateDbOperator.DbOperatorInstance(DBase);
                    //测试连接
                    if (!dbOperator.ServerIsThrough())
                    { throw new Exception("数据库连接失败!"); }
                    serverconfig = new ClientConfig() { ServerIP = dt.Rows[0]["ServerIP"].ToString(), Port = System.Convert.ToInt32(dt.Rows[0]["ServerPort"].ToString()), TimeOut = 10, ReceiveBufferSize = 128 };
